...
const ManageCategories = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        // Edit category
        await update(
          dbRef(database, `VTS/GeneralMaster/Product Group/${editId}`),
          {
        ...
        await set(dbRef(database, `VTS/GeneralMaster/Product Group/${newId}`), {
          id: newId,
          generalName: categoryName,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirm) return;
    try {
      await remove(dbRef(database, `VTS/GeneralMaster/Product Group/${id}`));
