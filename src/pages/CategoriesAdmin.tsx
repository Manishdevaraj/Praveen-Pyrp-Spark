...
const ManageCategories = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        // Edit category
        await update(
          dbRef(database, `SVT/GeneralMaster/Product Group/${editId}`),
          {
        ...
        await set(dbRef(database, `SVT/GeneralMaster/Product Group/${newId}`), {
          id: newId,
          generalName: categoryName,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirm) return;
    try {
      await remove(dbRef(database, `SVT/GeneralMaster/Product Group/${id}`));
